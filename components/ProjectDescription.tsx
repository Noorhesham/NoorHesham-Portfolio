...
const ProjectDescription = ({ project, moveback }: { project: any; moveback: () => void }) => {
  return (
    <div
      className="  h-full mb-12 mt-10 items-center justify-center  w-[80vw]"
      key={project.id}
    >
      <Button
        onClick={moveback}
      ...
      <div className=" relative flex items-center justify-center  w-[80vw] overflow-hidden   mb-10">
        <div className=" flex flex-wrap flex-col h-full lg:grid lg:grid-cols-2 gap-3  w-full  z-10 justify-between items-center">
          <img className=" h-auto rounded-xl" src={project.img} alt={project.title} />
          {project.video && (
            <iframe
              src={project.video}
              className="rounded-xl  w-full overflow-y-scroll "
              width="560"
              height="100%"
              scrolling="no"
